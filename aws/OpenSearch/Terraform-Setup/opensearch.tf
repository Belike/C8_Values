locals {
  opensearch_domain_name = "norman-tf-os" # Replace "domain-name" with your domain name

  opensearch_master_username = "camunda"    # Replace with your opensearch username
  opensearch_master_password = "P2TPy%biQ%Pb4E" # Replace with your opensearch password, password must contain at least one uppercase letter, one lowercase letter, one number, and one special character.
  ...
  camunda_optimize_service_account = "optimize-sa" # Replace with your Kubernetes ServiceAcccount that will be created for
}
...
output "opensearch_cluster" {
  value       = module.opensearch_domain.opensearch_cluster
